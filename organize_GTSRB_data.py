...
train_x = np.zeros((train_count, 50, 50, 3))
train_y = np.zeros((train_count,))

transformer = torchvision.transforms.RandomCrop((50,50), padding=False, pad_if_needed=True, padding_mode='symmetric')
train_x_index = 0
for i in range(0,43):
    digit_length = len(str(i))
    append_zero_count = train_file_digit_count - digit_length
    ...
    for j in range(1, len(Lines)):
        ...
        factor = max(50 / image.size[0], 50 / image.size[1])
        if factor < 1:
            factor = 1
...
test_x = np.zeros((test_count, 50, 50, 3))
